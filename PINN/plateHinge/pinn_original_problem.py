...
def GenDist(XY_dist):
    ...
    for i in range(len(XY_dist)):
        ...
        dist_s22[i, 0] = min(0.5 - XY_dist[i][1],XY_dist[i][1]) + np.exp(-((XY_dist[i][0]**2)/(delta**2)))
        dist_s12[i, 0] = min(XY_dist[i][1]+np.exp(-((XY_dist[i][0]**2)/(delta**2))), 0.5 - XY_dist[i][1] + np.exp(-((XY_dist[i][0]**2)/(delta**2))), 0.5 - XY_dist[i][0])
...
XY_ref= np.array([0.0, 0.0]) + np.array([0.02, 0.02]) * lhs(2, 20000)

XY_ref= np.concatenate((XY_ref,np.array([0.0, 0.48]) + np.array([0.02, 0.02]) * lhs(2, 20000)),0)
XY_c=np.concatenate((XY_c,XY_ref),0)
...
XY_c = np.concatenate((XY_c, LF[::2, :], RT[::2, 0:2], UP[::2, :], LW[::2, :]), 0)
...
class ANN_UvModel(nn.Module):
    ...
    def lossFct(self,xy,yh):
        ...
        loss = torch.mean(f_s11**2)+torch.mean(f_s22**2)+torch.mean(f_s12**2)+torch.mean(f_u**2)+torch.mean(f_v**2)
        return loss
...
model_uv.to(device)
train_loader = DataLoader(XY, batch_size=128,shuffle=True)
...
# LFBGS
train_loader = DataLoader(XY, batch_size=XY.shape[0],shuffle=True)
...
model_uv.to(device)
model_uv.optim = LBFGS(model_uv.parameters(), history_size=10, max_iter=1000)
EPOCHS=1000
model_uv.train(train_loader,EPOCHS)
...
def postProcessDef(xmin, xmax, ymin, ymax, field, s=5, num=0, scale=1):
    ...
    cbar = fig.colorbar(cf, fraction=0.046, pad=0.04, ax=ax[1], label=r'($m$)')
    cbar.ax.tick_params(labelsize=14)
    # cbar.ax.label(r'm')
    #
